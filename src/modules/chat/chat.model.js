...
export const ChatModel = {
  async create(title = "New Chat", mode = "gemini") {
    try {
      const db = getDB();
      const chat = {
        title,
  ...
  async getById(chatId) {
    try {
      ...
      const db = getDB();
      const chat = await db.collection(COLLECTION).findOne({ 
        _id: new ObjectId(idStr) 
      });

      if (!chat) {
        log.warn(`Chat not found for ID: ${idStr}`);
  ...
  async addMessage(chatId, userMessage, assistantMessage) {
    try {
      const db = getDB();
